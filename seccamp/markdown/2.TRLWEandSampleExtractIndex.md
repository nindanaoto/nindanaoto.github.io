...
- $N∈\mathbb{R}^+$とする
- TRLWE多項式環はTorusを係数とする多項式を$X^N+1$で割った余りが成す剰余環である
...
## 2.TRLWEの具体的構成(平文が$\mathbb{T}_N[X]$の場合)
...
## 2. TRLWEの具体的構成(平文が$\mathbb{B}_N[X]$の場合)

- これもTLWEと似た形
- $m∈ B[X],μ=1/8\in\mathbb{T}$とする。
- TLWEの暗号文は$b[X]=a[X]⋅ s[X]+μ(2⋅ m[X]-1[X])+e[X](μ(2⋅ m-1[X])∈\mathbb{T}_N[X])$
- 復号は$(1+\mathit{sgn}(b[X]-a[X]⋅ s[X]))/2$である。($\mathit{sgn}$は符号関数で各係数に作用)

---

## 2. SampleExtractIndex
...
## 2.SampleExtractIndexの具体的構成(数式表現)
